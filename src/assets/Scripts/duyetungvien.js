...
function Data_DuyetUngVien(data){
    ...
    for (let i = 0; i < data.length; i++) {
        if (data[i]["daduyet"]==-1){
            ...
            <td>
                <p class="fw-normal mb-1">${data[i]["hoten"]}</p>
            </td>
            <td>${data[i]["congviec"]}</td>
            <td class="format-td">
                ...
                <div class="format-td-baidang">
                    <a class="format-baidang" class="aa" role="button" href="../../../src/Views/admin/pageinfoadmin.php?idbaidang=${data[i]["MaDangTin"]}" class="btn btn-link btn-rounded">
                        Xem bài đăng
                    </a>
                </div>
